...
- Imports: explicit `.ts`/`.tsx` extensions on all relative imports (`allowImportingTsExtensions` is on; the bundler resolves them). `verbatimModuleSyntax` is on — use `import type` for type-only imports. `apps/web` and `apps/infra` resolve `#/*` to `src/*`; `apps/machine` and `apps/cli` use relative paths only.
...
- `apps/infra/docker/app.ts:72-73` builds with context `apps/` and Dockerfile `apps/Dockerfile` (neither exists). The real Dockerfile is `apps/web/Dockerfile`, and it copies root-level `package.json`/`bun.lockb`/`bunfig.toml` (`apps/web/Dockerfile:5-9`), so it expects the repo root as build context.
- `apps/infra/utils/run-command.ts:35-37` is fixed — resolves to the repo root via `resolve(import.meta.dir, "../../../")` where `Pulumi.yaml` lives.
- `apps/web/scripts/check-env.ts` does not exist; `apps/infra/utils/extract-env.ts:8-9` still cites it. `apps/web/package.json:16` correctly runs `bun ../infra/utils/run-command.ts`.
