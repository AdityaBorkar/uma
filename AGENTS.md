...
- `apps/machine` — `uma-machine`: CLI + daemon that runs Tasks in microsandbox sandboxes (Bun + SQLite). `server-central/` is a local contract test server, not production. Read `apps/machine/docs/ARCHITECTURE.md` for the module/port/dependency map, plus `apps/machine/docs/CONTEXT.md` and `apps/machine/docs/DOMAIN-MODELS.md`.
