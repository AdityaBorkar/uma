...
- Imports: explicit `.ts`/`.tsx` extensions on all relative imports (`allowImportingTsExtensions` is on; the bundler resolves them). `verbatimModuleSyntax` is on — use `import type` for type-only imports. `apps/web` resolves `#/*` to `src/*`; `apps/machine` and `apps/cli` use relative paths only; `apps/infra` declares `#/*` → `./src/*` in `package.json:5-7` but has no `src/` dir — code uses relative imports (`apps/infra/index.ts:4-8`).
