...
- `utils/extract-env.ts` is the single source of truth for container env (`APP_ENV_VARS` manifest + `appEnvValues`/`appBuildArgs`/`appRuntimeEnvs`/`webRuntimeEnvs`/`extractEnv`). When adding a var: add it here (correct `source`: `app`|`postgres`|`derived`; `secret` for secrets; `optional` for unset-tolerant; `build: true` only for `PUBLIC_*`), then update validation (`apps/web/src/env.ts` for web vars, `apps/server/src/env.ts` for server vars) and `apps/web/Dockerfile` (`ARG` only for `PUBLIC_*`; secrets runtime-only, never build args — `appBuildArgs` excludes them by construction; the server image takes no build args at all). The web container is secret-free (`webRuntimeEnvs`: `PUBLIC_*` + derived `CONTROL_PLANE_URL`); the server container gets the full set (`appRuntimeEnvs`).
...
- Wiring is verified, not trusted: `docker/web.ts` + `docker/server.ts` build from the repo-root context with `apps/web/Dockerfile` / `apps/server/Dockerfile`; `utils/run-command.ts` resolves the repo root where `Pulumi.yaml` lives; `bun run check:env` in `apps/web` verifies the env manifest against the web Dockerfile and `apps/web/src/env.ts`. Caddy routes `/api/*` to the server container, everything else to web. Fix paths when you touch them; don't entrench them.
