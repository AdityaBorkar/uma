...
- Imports use `#/*` → `src/*` alias with explicit `.ts` extensions, `import type` for types (`verbatimModuleSyntax`, strict TS).
...
- Known drift (verify before trusting): `docker/app.ts` references a non-existent `apps/infra/Dockerfile` and wrong build context (real file: `apps/web/Dockerfile`, expects repo-root context); `utils/run-command.ts` resolves the project dir to `apps/` instead of the repo root (works only because Pulumi walks up). Fix paths when you touch them; don't entrench them.
