...
5. **Watch in the browser** (`apps/web`) — create a Task (`queued`), watch it go `running → completed | failed` as `log` / `task-done` frames arrive over `/api/machines/ws`. Enroll devices at `/device`, manage machines at `/settings/machines`. Web procedures: [/contracts/server-web](/contracts/server-web).
...
| --- | --- |
| `apps/web` | Product app (routes in `src/routes/`, oRPC in `src/rpc/`, machine server in `src/lib/machines/` + `src/routes/api/machines/ws.ts`, schemas in `src/schemas/db/`) |
...
Env/build wiring is verified by `bun run check:env` in `apps/web` (manifest ↔ Dockerfile ↔ `src/env.ts`).
