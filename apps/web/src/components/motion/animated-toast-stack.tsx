...
export function useAnimatedToastStack({
	...
	const updateToast = useCallback((id: string, patch: Partial<ToastInput>) => {
		setToasts((current) =>
			current.map((toast) =>
				toast.id === id
					? // exactOptionalPropertyTypes: the spread widens optionals to `| undefined`.
						({
							...toast,
							...patch,
							createdAt:
	...
	useEffect(() => {
		...
		toasts.forEach((toast) => {
			...
			toastTimers.current.set(toast.id, { signature, timer });
		});
...
const ToastItem = memo(function ToastItem({
	...
	toast: AnimatedToast;
	index: number;
	// exactOptionalPropertyTypes: callers pass possibly-undefined through, so
	// these accept (but never require) undefined. Runtime behavior unchanged.
							...
							<AnimatePresence initial={false} mode="popLayout">
								<motion.span
									...
									exit={
										reduce
											? { opacity: 0 }
											: { filter: "blur(6px)", opacity: 0, scale: 0.9, y: -8 }
									}
									initial={
										reduce
											? { opacity: 0 }
