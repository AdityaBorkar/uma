...
export function Tooltip({
	...
	const place = useCallback(() => {
		...
		const point: Record<Side, { top: number; left: number }> = {
			bottom: { left: cx, top: (anchorPoint ? cy : r.bottom) + GAP },
			left: { left: (anchorPoint ? cx : r.left) - GAP, top: cy },
			right: { left: (anchorPoint ? cx : r.right) + GAP, top: cy },
			top: { left: cx, top: (anchorPoint ? cy : r.top) - GAP },
		};
