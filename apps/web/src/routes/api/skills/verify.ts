...
import { isSafeSkillSource } from "#/lib/skills/skills.ts";
 ...
 * `execFile` with no shell; the source is allowlisted to printable,
 * whitespace-free values first.
 */
export const Route = createFileRoute("/api/skills/verify")({
	server: {
		handlers: {
			GET: async ({ request }) => {
				...
				if (
					/failed to clone|installation failed|canceled|cancelled/i.test(clean)
				) {
					const hint = clean
						.split("\n")
						.map((l) => l.trim())
