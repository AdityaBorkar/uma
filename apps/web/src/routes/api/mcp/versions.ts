...
import { isSafeMcpPackage } from "#/lib/mcp/mcp.ts";
 ...
 * npm package via the public registry. Used by the MCP servers settings page
 * to pin and upgrade local servers (`pkg@version`).
 */
export const Route = createFileRoute("/api/mcp/versions")({
	server: {
		handlers: {
			GET: async ({ request }) => {
