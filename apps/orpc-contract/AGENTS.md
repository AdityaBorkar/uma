...
- Layout: `src/index.ts` barrel, `src/constants.ts` (`PROTOCOL_VERSION`, task sizing, heartbeat cadence/retention, log cap, pressure policy, ws path, reserved names), `src/ids.ts`, `src/schemas/` (machine-frames, server-frames, primitives), `src/contracts/`, `src/utils.ts` (`needsUpgrade`/`compareVersions`, `branchForTask`, `quotaDefaultsFromRam`). Keep this layout; both consumers import the barrel, not deep paths.
- Frozen means frozen: frame names (`t`), required fields, validation semantics, constants, `protocol: "v1"`, `projectId: null` = global scope, log-cap semantics, branch/quota defaults.
...
- Machine side must keep ignoring unknown inbound `t` and must never send unvalidated frames (`validateMachineFrame` on send). Use `semver` with the naive numeric fallback in `utils.ts` — don't introduce a second version-compare.
