...
/**
 * Builds, persists, and queries heartbeat frames. Collectors are Linux-first
 * (/proc + df); sandbox telemetry degrades gracefully when the runtime is
 * absent. One instance per daemon loop.
 */
export class Heartbeat {
	/** Build one frame: host metrics + sandbox listing + pressure scope hint. */
	async build(
		...
		if (
			...
			try {
				const per = await Sandbox.metricsForPressure();
	...
	/** Persist-then-send: the raw frame lands in SQLite before any send. */
	persist(frame: HeartbeatFrame, ts = Date.now()): void {
	...
	/** History query over the window; [] on a fresh machine (nothing recorded). */
	history(range: "24h" | "30d" | string, limit?: number): HeartbeatSample[] {
		const windowMs =
	...
	private async hostMetrics(extraPids: number[] = []): Promise<HostMetrics> {
	...
	private async ram(): Promise<number> {
		try {
	...
	private async disk(path = "/"): Promise<number> {
	...
	private async sandboxInfos(): Promise<SandboxInfo[]> {
		try {
...
export async function buildHeartbeat(
	...
	return new Heartbeat().build(quotaUsage, scopeHint);
}

export function persistHeartbeat(frame: HeartbeatFrame, ts = Date.now()): void {
	new Heartbeat().persist(frame, ts);
}
...
export function readHistory(
	...
	return new Heartbeat().history(range, limit);
}
