...
- Relative imports only, explicit `.ts` extensions, `import type` for types. No `#/*` alias here (that is web-only; infra declares it but uses relative — see `apps/infra/AGENTS.md`). Never import from `apps/web` or other consumer apps — only `@uma/orpc-contract` (workspace dep) plus npm/catalog deps.
...
- Validation with Zod at trust boundaries: inbound ws frames via `parseServerFrame` (unknown `t` → `null`, logged + ignored), every outbound `send` via machine-side `assertMachineFrame` (`src/execution/protocol.ts:15`) wrapping `validateMachineFrame`; `identity.json`/`limits.json`, machine names, `--only` lists.
...
- SQLite (`src/utils/db.ts`, `src/utils/client.ts`, `src/schemas/db/`): additive DDL only (`CREATE TABLE/INDEX IF NOT EXISTS` + `ADD COLUMN`), `PRAGMA user_version` mirrors `SCHEMA_VERSION`, WAL mode, `migrate()` in `src/utils/client.ts:102-109` runs at startup (there is no migration script).
- DI without a framework: function-arg injection (`new ExecutionEngine({agentBin?, claim?}).executeTask(assign, emit)` per `src/execution/execution.ts:55-59,94-97`, `connectWithBackoff(handlers, { shouldStop, … })`). Tests inject `emit`/`claim` and use the mock driver. `evaluateScopeHint` is pure/clock-free; `resolveLimits` in `src/enrollment/limits.ts:22-44` is the single quota resolver (server > file > default).
