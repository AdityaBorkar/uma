...
`stop --force` / `remove --force` verbs). Driver order is mock-if-flag (`MSB_MOCK=1`/`UMA_MSB_MOCK=1` forces mock per `src/sandboxes/msb/mock.ts:16-20` and `src/sandboxes/msb/driver.ts:65-79`) else SDK → CLI → fail-closed. We chose this over bundling natives or dropping
