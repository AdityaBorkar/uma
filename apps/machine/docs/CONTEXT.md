...
The `uma` server (Tasks, Signals, Projects, machine registry) is an external upstream system, not a bounded context owned here. This repo conforms to its `v1` contract via `orpc-contract/`.
