...
`src/` ownership (see `docs/plans/remote-machine.md §12`):
...
- **CLI**: one-shot commands `enroll|daemon|check|reset|sync|history|sandbox|run|version` (`src/index.ts:197`). Exit `0` ok, `2` drifted/partial, `1` error, `3` upgrade-required.
...
- **WsClient**: one ws per machine, Bearer auth, jittered backoff (`src/ws-client.ts:41`), logical `heartbeat|claim|logs|config` channels multiplexed as v1 JSON frames (no explicit channel field on the wire).
- **HeartbeatCollector**: `collectHostMetrics` + `listSandboxes` + `sandboxMetricsForPressure` → `buildHeartbeat` → `persistHeartbeat`.
- **SandboxDrivers**: `driver()` selects `sdk` → `cli` → fail closed (`src/sandbox/driver.ts`); mock only when `MSB_MOCK=1`/`UMA_MSB_MOCK=1`, file-backed (`mock-sandboxes.json` + `msb-root-meta/<name>/meta.json` under `dataDir`, tests/dev). SDK = streaming + `secretEnv`; CLI = `msb create/start/stop --force/remove --force/exec`. True streaming is SDK-only; CLI/mock funnel through capture.
- **ConfigConvergers**: 9 `KeyModule {KEY, check, reset}` run in `ORDERED_KEYS` order; `resolveKeys(--only)` filters; `desired.json` corruption is reported as drift, not silently defaulted.

## Service

Two senses, kept distinct:

- Domain services: `executeTask`, `performSync`, `evaluateScopeHint`, `ensureBinding` (see `docs/DOMAIN-MODELS.md#domain-service`).
...
- Store port: `withDb` + `insert*/query*/persist*/record*` (`src/db.ts`).
...
- **Sandbox adapters**: `src/sandbox/{sdk,cli,mock}.ts` implement the same `SandboxDriver` port (`create/start/stop/remove/list/exec/execStream/metrics`); `task.id`/`project.id` labels set on SDK + CLI create (`user.id` never set — `executeTask` passes no userId).
- **Store adapter**: `db.ts` over `drizzle-orm` + `bun:sqlite` (WAL, `0600` incl sidecars, `withDb` open/close, `drizzle/` migrations).
...
- Isolation: Tasks only inside `msb` (pinned Ubuntu); no host fallback; `sync` never touches worktrees. Per-Task `--net/--no-net` (default deny) is plan §11 policy, not implemented in code — no net flags are passed at create today.
...
- Files: `identity.json {machineId, serverUrl, sessionToken 0600, enrolledAt, machineName?}`, `limits.json {ramGB, maxRunning, maxTotal, computedAt, source:"install-probe"}` (`2×/5×` GB, floor 1), `state.db` (SQLite), desired cache (`desired.json` via `loadDesired/saveDesired`: `version` + `templates`/`limits`/`programs`/`providers`/`agents`/`mcp`/`skills`).
...
- Permissions: `chmod0600` on `identity.json`, `state.db` + `-wal/-shm/-journal` (`src/fs-utils.ts`).
...
- Retention: 30d heartbeats + vacuum only when deletes (`src/db.ts:254`), 90d events/receipts, 7d log buffer (`src/db.ts:276` + `pruneAuxTables`).
