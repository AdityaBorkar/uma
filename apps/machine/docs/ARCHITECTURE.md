...
- `enrollment/`: `enroll.ts` — device flow + `identity.json` + `limits.json` + SQLite init + systemd writer (`src/enrollment/enroll.ts:208-228` does no Ubuntu pre-pull; no `install.sh` exists in repo); `limits.ts`: single quota resolver (`resolveLimits`, server > file > default).
...
- `sandboxes/`: `sandbox.ts` — `Sandbox` handle class (create/start/stop/remove/list/exec/execStream/metrics; statics + instance ops route through an injected `DriverSelector`, default `defaultSelector`) + `snapshotQuota` + quota helpers. Drivers: `sandboxes/msb/sdk.ts` (`MsbSdkDriver`), `sandboxes/msb/cli.ts` (`MsbCliDriver`), `sandboxes/msb/mock.ts` (`MsbMockDriver`, constructor-injectable paths), `sandboxes/msb/driver.ts` (`DriverSelector` — mock only if `MSB_MOCK`/`UMA_MSB_MOCK` set else SDK → CLI → fail-closed per `src/sandboxes/msb/driver.ts:65-79`; sticky availability probes, fail-closed; `sandboxes/msb/shared.ts`, `sandboxes/msb/types.ts`).
- `coding-agents/`: `registry.ts` + `omp/` + `opencode/` + `types.ts` — agent selector (`UMA_CODING_AGENT` in `src/coding-agents/registry.ts:13-17`).
- `config/`: `mod.ts` + `desired.ts` (desired-state cache load/save) + `key.ts` (`BaseConfigKey`) + `skills-installer.ts` + `<key>.ts` (8 keys in `ORDERED_KEYS` order per `src/config/mod.ts:16-25`; file `git.ts` exports KEY `git-login`, file `skills.ts` exports KEY `skills`) + `sync.ts`: check/reset + receipts.
...
- **WsClient**: one ws per machine, Bearer auth, jittered backoff (`src/daemon/ws-client.ts`), machine→server `t` = `heartbeat|log|task-done|check-ack|reset-ack|sync-ack|claim-ack|quota-exceeded` (`apps/orpc-contract/src/schemas/machine-frames.ts:116-125`), server→machine `assign|cancel|reset-config|UPGRADE_REQUIRED` (`apps/orpc-contract/src/schemas/server-frames.ts:55-60`) multiplexed as v1 JSON frames (no explicit channel field on the wire).
- **HeartbeatCollector**: `collectHostMetrics` + `listSandboxes` + `sandboxMetricsForPressure` → `buildHeartbeat` → `persistHeartbeat`.
- **SandboxDrivers**: `DriverSelector.resolve()` selects mock only if `MSB_MOCK`/`UMA_MSB_MOCK` set (`src/sandboxes/msb/mock.ts:16-20`) else `sdk` → `cli` → fail closed (`src/sandboxes/msb/driver.ts:65-79`); sticky availability probes are instance state (fresh selector or `reset()` in tests); mock file-backed (`mock-sandboxes.json` + `msb-root-meta/<name>/meta.json` under `dataDir` — mock-only, `src/sandboxes/msb/mock.ts:55-61`, tests/dev; paths constructor-injectable on `MsbMockDriver`). SDK = streaming + `secretEnv`; CLI = `msb create/start/stop --force/remove --force/exec`. True streaming is SDK-only; CLI/mock funnel through capture.
- **ConfigConvergers**: 8 `KeyModule {KEY, check, reset}` run in `ORDERED_KEYS` order (`src/config/mod.ts:16-25`); `resolveKeys(--only)` filters (alias `"sync"` → `"skills"`); `desired.json` corruption is reported as drift, not silently defaulted.
...
- OS service: `uma-machine.service` user unit (`Restart=always`, `WantedBy=default.target`) + `loginctl enable-linger` owned by reset/sync (`src/config/systemd.ts:89-124` mutates; `check()` at `src/config/systemd.ts:65-87` is read-only probes). Logs to journald; foreground `daemon` for dev.
...
`config/* → desired, utils/db, env, proc, fs-utils`; `config/mod → 8 keys`; `config/providers → utils/db, redact`
`all → orpc-contract` (contract has zero deps on `src/`); `sandbox → env, proc, sandboxes/msb/*`; `limits → enroll + orpc-contract`
`utils/db → drizzle-orm + ms + schemas/db + utils/client (`src/utils/db.ts:1-31`); `ws-client → enroll(identity) + orpc-contract + execution/protocol (`assertMachineFrame`, `src/daemon/ws-client.ts:1-4`).
...
- Server port: ws frames + `POST /api/machines/claim` (`ExecutionEngine` claim in `src/execution/execution.ts:363-375`, injectable via `ExecutionEngineOptions.claim`; 409 + other 4xx → `rejected`, 5xx/retry-exhausted → `unreachable` per `src/execution/execution.ts:378-396`; non-ok frees the sandbox).
...
- HTTPS: `POST /api/machines/claim` atomic (`UPDATE … WHERE status='queued'` server-side; claim result decides here: `ok` vs `rejected` vs `unreachable`).
- CLI UX: tables + `--json`, `--only`, `--dry-run`, `--prune` (default false), exit codes above; `history [--range <ms-string>]` (any `ms` string per `src/daemon/heartbeat.ts:146-151`) reads 30d raw.

## Protocol

Wire detail (`src/daemon/ws-client.ts`, `apps/orpc-contract/src/schemas/machine-frames.ts`, `apps/orpc-contract/src/schemas/server-frames.ts`):

1. `enroll` → `device.code` → print `user_code + verification_uri_complete` (`src/enrollment/enroll.ts:139-142`) → poll `device.token` (`authorization_pending/slow_down/expired_token/access_denied`) → store Bearer `0600`.
...
3. Server sends `assign{taskId, projectId|null, prompt, repoUrl?="", branch?, prompt, limits?, commit?, freshStart?}` → client `quotaPreCheck` → `create → claim-ack → start → ensureBinding → execStream(redacted, 256KB, buffered) → task-done{projectId} → stop` (idle `stopped` left for TTL reap; never removed in `executeTask`).
...
- Filesystem: full XDG (`configDir/dataDir/identityPath/limitsPath/stateDbPath` in `src/utils/env.ts` via `env-paths` `osConfigDir/osDataDir` per `src/utils/env.ts:20-26`; host `~/` only via `homedir()` fallback, never hardcoded); `msb` root owned by runtime under `~/.microsandbox/`; mock-only meta under `dataDir/msb-root-meta/` (`src/sandboxes/msb/mock.ts:55-61`).
...
`src/index.ts:main()` (CLI) + `new Daemon().run()` (long-lived). Manual wiring, no container:

- `main` parses args → calls `enroll / new Daemon().run() / checkAll / resetAll (+ freshStart) / performSync / new Heartbeat().history() / Sandbox.list() (+ prune via new Sandbox().remove()) / new ExecutionEngine().executeTask` with explicit args (`src/index.ts:157,200,227,250-253,307`).
...
- `new ExecutionEngine(opts).executeTask(assign, emit)` (`src/execution/execution.ts:55-59,94-97`) — admission lock, in-flight cancel state, and refusal frames live inside the module; tests inject `emit` + `claim` and use the mock driver.
...
- Env access lives in `utils/env.ts` helpers, but is not exclusive: `sandboxes/msb/mock.ts` reads `MSB_MOCK`/`UMA_MSB_MOCK`, `execution/execution.ts` reads `GH_TOKEN`/`GITHUB_TOKEN` + `UMA_AGENT_BIN`, `config/systemd.ts` reads `USER`/`LOGNAME`/`UMA_SYSTEMD_UNIT`/`UMA_EXEC_PATH`/`XDG_CONFIG_HOME` directly (not via `env.ts`, `src/config/systemd.ts:11-12,28,67,95`), `enrollment/enroll.ts` reads `UMA_RAM_GB`, `coding-agents/registry.ts:15` reads `UMA_CODING_AGENT`. Secrets passthrough (`exportProviderEnv`, `MSB_PATH`) intentionally bypasses `utils/env.ts`.
...
- Env (`src/utils/env.ts` + call sites): `UMA_SERVER_URL` (+ `UMA_SERVER` fallback), `UMA_TOKEN_FILE`, `UMA_MACHINE_ROOT`, `UMA_CONFIG_HOME`/`UMA_DATA_HOME` (full-dir overrides; base dirs via `env-paths` per `src/utils/env.ts:20-26`) vs `XDG_CONFIG_HOME`/`XDG_DATA_HOME` (base), `UMA_HEARTBEAT_INTERVAL_S` (≥5, default 30; daemon `--interval` floor is 2s via `daemonIntervalS`), `UMA_HEARTBEAT_RETENTION_DAYS` (≥1, default 30), `UMA_STATE_DB`, `UMA_MSB_BIN`/`MSB_PATH`, `UMA_SANDBOX_TTL_S` (≥60, default 3600), `UMA_AGENT_BIN`, `UMA_CODING_AGENT` (`src/coding-agents/registry.ts:15`), plus `UMA_RAM_GB` (enroll probe fallback), `UMA_MSB_MOCK`/`MSB_MOCK`, `UMA_HOME` (files/skills), `UMA_EXEC_PATH`/`UMA_SYSTEMD_UNIT` (systemd), `GH_TOKEN`/`GITHUB_TOKEN` (redaction).
