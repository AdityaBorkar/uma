...
- `db.ts` (store facade) + `db/schema.ts` (tables) + `db/client.ts` (open/migrate/WAL) + `db/migrations.ts` (GENERATED: drizzle-kit SQL from `./drizzle` embedded for the compiled binary, applied via drizzle-orm embedded-journal mode to the XDG state.db, tracked in `__drizzle_migrations`; `PRAGMA user_version` mirrors the applied count): SQLite stores + retention. `redact.ts`: secrets + 256KB split. `env.ts`: XDG + `UMA_*` resolution. `protocol.ts`: re-export of `orpc-contract` + validated frame/refusal helpers. `proc.ts`: process runner (`runCapture`/`whichBin`). `limits.ts`: single quota resolver. `version.ts`: `CLI_VERSION`/`CONFIG_VERSION`. `fs-utils.ts`: `chmod0600`, parent-dir helpers.

## Package

- Runtime deps (`package.json`): `zod` (validation), `microsandbox` SDK (in-process sandbox API), plus CLI/store/util deps (`cac`, `cli-table3`, `drizzle-orm`, `env-paths`, `es-toolkit`, `fast-redact`, `ms`, `nanoid`, `p-retry`, `picocolors`, `semver`, `write-file-atomic`, `zod-validation-error`). Dev: `@types/*`, `typescript`, `zod-to-json-schema`, `drizzle-kit` (migration generation only).
...
- **Store adapter**: `db.ts` + `db/schema.ts` + `db/client.ts` + `db/migrations.ts` over `drizzle-orm` + `bun:sqlite` (WAL, `0600` incl sidecars, `withDb` open/close, drizzle-kit SQL embedded via `bun run db:codegen` and applied to the XDG state.db on every writable open).
